...
const { VISIBILITY: CONTENT_VISIBILITY } = require('../lib/content-visibility');
...
function _contentAccessLevel(visibility) {
  return {
    [VISIBILITY.PRIVATE]: CONTENT_VISIBILITY.PRIVATE,
    [VISIBILITY.WORKSPACE_SHARED]: CONTENT_VISIBILITY.WORKSPACE_SHARED,
    [VISIBILITY.ORGANIZATION_SHARED]: CONTENT_VISIBILITY.ORGANIZATION_SHARED,
    [VISIBILITY.PLATFORM_TEMPLATE]: CONTENT_VISIBILITY.PLATFORM_TEMPLATE,
...
function approveOrganizationPublication({ replayId, workspaceId, userId } = {}) {
  ...
  const tx = db.transaction(() => {
    ...
    `).run(replayId, ws);
    if (replay.content_id) {
      db.prepare('UPDATE content SET access_level=? WHERE id=?')
        .run(_contentAccessLevel(VISIBILITY.ORGANIZATION_SHARED), replay.content_id);
    }
    return _bumpRevision(ws);
  });
  const revision = tx();
