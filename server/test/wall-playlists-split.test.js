...
const wallPlaylists = require('../lib/wall-playlists');
...
test('split wall members fork their shared playlist into private playlists', () => {
  const prefix = `test-wall-playlists-${Date.now()}-`;
  ...
  try {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, 2, 1, ?, NULL, 'split')
    `).run(wallId, userId, workspaceId, 'Split Wall', wallPlaylistId);
    db.prepare(`
    ...
    const cloneA = wallPlaylists.ensureDevicePlaylist(d1, userId);
    const cloneB = wallPlaylists.ensureDevicePlaylist(d2, userId);

    assert.notEqual(cloneA, wallPlaylistId, 'first display forks off the shared wall playlist');
    assert.notEqual(cloneB, wallPlaylistId, 'second display forks off the shared wall playlist');
    assert.notEqual(cloneA, cloneB, 'each display gets its own private playlist');
    ...
    assert.equal(afterA, cloneA);
    assert.equal(afterB, cloneB);
    ...
    assert.deepEqual(wallItems.map(r => r.content_id), [oldContent], 'shared wall playlist remains intact');

    const cloneAItems = db.prepare('SELECT content_id FROM playlist_items WHERE playlist_id = ? ORDER BY sort_order').all(cloneA);
    const cloneBItems = db.prepare('SELECT content_id FROM playlist_items WHERE playlist_id = ? ORDER BY sort_order').all(cloneB);
    assert.deepEqual(cloneAItems.map(r => r.content_id), [oldContent], 'first private playlist starts with wall content');
    assert.deepEqual(cloneBItems.map(r => r.content_id), [oldContent], 'second private playlist starts with wall content');
  } finally {
    cleanup(prefix);
  }
});
