...
test('Media Control visibility changes are explicit and cannot bypass organization approval', async () => {
  ...
  assert.equal(content.access_level, 'workspace_shared');
...
test('non-numeric storage error codes fail as JSON HTTP 500 responses', async () => {
  const original = svc.setVisibility;
  svc.setVisibility = () => {
  ...
  try {
    const response = await request(`/${ws1Replay}/visibility`, {
      method: 'PATCH', role: 'editor', body: { visibility: 'PRIVATE' },
    });
    assert.equal(response.status, 500);
    assert.equal(response.payload.error, 'forced storage failure');
  } finally {
    svc.setVisibility = original;
  }
});
