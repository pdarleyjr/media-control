...
async function startLiveStream(btn, refreshAfterSend) {
  ...
  try {
    const result = await api.liveStream.start({ initiator: 'operator' });
...
async function clearLiveContent(btn, refreshAfterSend) {
  ...
  try {
    ...
    if (!composition?.content_instance_id) {
      showToast(t('mc.live.composition.no_content'), 'info');
      return;
    }
    await api.liveStream.compositionClear({
      content_instance_id: composition.content_instance_id,
      expected_compositor_revision: composition.revision,
      idempotency_key: globalThis.crypto?.randomUUID?.()
        || `command-bar-remove-${Date.now()}`,
    });
...
export function renderCommandBar(container, {
  ...
  const syncLiveControls = async () => {
    try {
      ...
      const active = status?.publisher?.active === true;
      if (liveStartBtn) liveStartBtn.hidden = active;
      if (liveStopBtn) liveStopBtn.hidden = !active;
      if (liveClearBtn) liveClearBtn.hidden = !active || !status?.composition?.content_instance_id;
    } catch {
