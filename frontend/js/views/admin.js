...
export async function render(container) {
  ...
  container.innerHTML = `
    ...
    <details class="settings-section" id="operationalDiagnostics">
      ...
      <p style="color:var(--text-muted);font-size:12px;margin:10px 0">Read-only bounded software telemetry. Opening this panel does not contact or command classroom devices; it does not verify physical pixels, audio, or touch.</p>
...
async function loadOperationalDiagnostics() {
  ...
  try {
    ...
    body.innerHTML = `
      ...
      <div class="table-wrap"><table style="width:100%;border-collapse:collapse;font-size:12px;min-width:1080px">
        <thead><tr style="border-bottom:1px solid var(--border);text-align:left"><th style="padding:8px">Renderer</th><th style="padding:8px">Connection</th><th style="padding:8px">Playback (software)</th><th style="padding:8px">Last software progress</th><th style="padding:8px">Command / ACK</th><th style="padding:8px">Content</th></tr></thead>
        <tbody>${renderers.map((renderer) => `
          <tr style="border-bottom:1px solid var(--border)"><td style="padding:8px"><strong>${esc(renderer?.name || renderer?.id || 'Unknown')}</strong><div style="color:var(--text-muted)">${esc(renderer?.id || '')}</div></td><td style="padding:8px">${diagnosticStatus(renderer?.connected)}<div style="color:var(--text-muted)">${esc(formatAge(renderer?.heartbeat_age_sec))}</div></td><td style="padding:8px">${esc(renderer?.render_progress?.playback_state || renderer?.latest_render_confirmation?.state || 'unknown')}<div style="color:${renderer?.render_progress?.error?.active ? 'var(--danger)' : 'var(--text-muted)'}">${esc(renderer?.render_progress?.error?.active ? renderer.render_progress.error.message : (renderer?.render_progress?.decoded_frame_available ? 'decoded frames available' : 'media-time fallback'))}</div></td><td style="padding:8px">${esc(formatTime(renderer?.render_progress?.last_confirmed_render_progress_at))}<div style="color:var(--text-muted)">${esc(renderer?.render_progress?.physical_pixels_observed === false ? 'software only; no physical-pixel claim' : 'unavailable')}</div></td><td style="padding:8px">${esc(renderer?.last_command?.command_id || 'No command correlation')}<div style="color:var(--text-muted)">ACK: ${esc(formatTime(renderer?.last_command?.acknowledged_at))} · ${esc(renderer?.last_command?.ack_latency_ms == null ? 'latency unavailable' : `${renderer.last_command.ack_latency_ms} ms`)}</div><div style="color:var(--text-muted)">Render: ${esc(formatTime(renderer?.last_command?.render_confirmation_at))}</div></td><td style="padding:8px">${esc(renderer?.content?.type || 'Unknown')}<div style="color:var(--text-muted)">${esc(renderer?.content?.id || 'No content ID')}</div></td></tr>
        `).join('') || '<tr><td colspan="6" style="padding:12px;color:var(--danger)">No renderer telemetry is available.</td></tr>'}</tbody>
      </table></div>
